...
[TestFixture]
internal class AddTeacherCommandValidatorTests
{
    private AddTeacherCommandValidator _validator;

    private static AddTeacherCommand CreateValidModel =>
        new(
            "ID12345",
            "John",
            "Doe",
            "TJ8",
            new DateOnly(1995, 4, 15),
            "Prof",
            1000
            );
    ...
    public void SetUp()
    {
        _validator = new AddTeacherCommandValidator();
    }
    ...
    [Test]
    public async Task WhenValidModelWithNoSalary_ThenShouldNotHaveAnyValidationErrors()
    {
        var cmd = CreateValidModel with { Salary = null };
        var result = await _validator.TestValidateAsync(cmd);
        result.ShouldNotHaveAnyValidationErrors();
    }
    ...
    {
        var cmd = CreateValidModel with { DateOfBirth = new DateOnly(2005, 1, 5) };
        var result = await _validator.TestValidateAsync(cmd);
